...
from model.pyramidnet_shakedrop import PyramidNetShake
...
def main():
    ...
    if args.model == 'resnet':
        ...
        model = PyramidNetShake(depth=32, alpha=295, bottleneck=True)
    ...
    optimizer = SAM(optional_weight_decay_param(model, args.weight_decay), torch.optim.SGD, lr=args.lr,
...
def train(train_loader, model, optimizer, criterion):
    ...
    for i, data in enumerate(train_loader):
        ...
        if is_cutmix:
            _loss = criterion(_output, target_a) * lam + criterion(_output, target_b) * (1. - lam)
        else:
            _loss = criterion(_output, target)

        _loss += model.reg_loss(alpha=0.1)
