...
class ModelProfiler():
    ...
    def collect_results(self):
        ...
        self.total_parameter_count = sum(
            result["parameter_count"] for result in self.module_results.values() if result["is_leaf"])
        self.total_flop_count = sum(
            result["flop_count"] for result in self.module_results.values() if result["is_leaf"])
    ...
    def print_results(self):
        ...
        for (module_name, module_result) in self.module_results.items():
            ...
            row = (
                ...
                utils.human_readable_int(parameter_count) if parameter_count is not None else "",
                utils.human_readable_int(flop_count) if flop_count is not None else "",
            )
            rows.append(row)
