...
class Conv2dFilter(ModuleFilter):
    def __init__(self, kernel_sizes=None):
        ...
        if self.kernel_sizes is not None:
            self.kernel_sizes = tuple(utils.force_two_tuple(kernel_size) for kernel_size in self.kernel_sizes)
...
class RegularizationMethodTransformer(ModuleTransformer):
    def apply(self, module, name, full_name):
        module.reg_loss = types.MethodType(modules.BSConvS_ModelRegLossMixin.reg_loss, module)
        return module
...
class ModuleReplacer():
    ...
    def add_rule(self, *args):
        if (len(args) == 1) and isinstance(args[0], ModuleReplacementRule):
            ...
            raise TypeError(
                "Rule must be specified either as instance of ModuleReplacementRule or as pair of ModuleFilter and ModuleTransformer instances")
    ...
    def apply(self, module):
        ...
        if self.verbosity >= 1:
            parameter_count_before = profile.ModelProfiler.count_parameters(module=module)
        ...
        if self.verbosity >= 1:
            ...
            parameter_count_after = profile.ModelProfiler.count_parameters(module=module)
            print("Model parameters changed from {} to {} (= {:.2f}%)".format(
                utils.human_readable_int(parameter_count_before),
                utils.human_readable_int(parameter_count_after),
    ...
    def _apply_recursively(self, module, name_prefix):
        ...
        for (child_name, child) in named_children:
            ...
            (child_replaced_count, new_child) = self._apply_rules(module=child, name=child_name,
                                                                  full_name=child_full_name)
            if child_replaced_count == 0:
                # if no rule applied, recurse into child module
                (child_replaced_count, new_child) = self._apply_recursively(module=child,
