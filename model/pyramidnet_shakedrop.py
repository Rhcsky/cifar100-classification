...
from model.shake_drop import ShakeDrop
...
class PyramidNetShake(nn.Module):

    def __init__(self, depth, alpha, bottleneck=True):
        super(PyramidNetShake, self).__init__()
        ...
        self.fc = nn.Linear(self.final_featuremap_dim, 100)
        ...
        for i in range(1, block_depth):
            ...
            layers.append(
                block(int(round(self.featuremap_dim)) * block.outchannel_ratio, int(round(temp_featuremap_dim)), 1,
                      p_shakedrop=self.ps_shakedrop[self.idx_shakedrop]))
